<html>
	<head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width,initial-scale=1">
		<title>Lazar Djokovic | Portfolio</title>
		<link rel="stylesheet" href="css/main.css"/>
        <link rel="shortcut icon" href="img/shortcutIcon/shortcutIcon.png"/>
        <script src="js/jquery-3.1.1.min.js"></script>
        <script type="text/JavaScript" src="js/main.js"></script>
	</head>
	<body>
		<header>
                <a href="index.php"><img src="img/praktikumLogo.jpg" alt="logo"/></a>
		</header>
		<nav>
			<div class="topnav" id="myTopnav">
              <a href="index.php"  class="current"><strong>Home</strong></a>
              <a href="about.php"><strong>About</strong></a>
              <a href="contact.php"><strong>Contact</strong></a>
              <a href="javascript:void(0);" class="icon" onclick="myFunction()">&#9776;</a>
            </div>
            <div class="topnav" id="myTopnavLarge">
              <a href="index.php" class="current"><strong>Home</strong></a>
              <a href="about.php"><strong>About</strong></a>
              <a href="contact.php"><strong>Contact</strong></a>
            </div>
		</nav>
		<section id="main">
			<div id="projects">
				<div id="titleProjects">
					<h1>-- My projects --</h1>
                    <hr/>
				</div>
				<div id="projectsImages">
					<div id="leftProjectImage" >
						<a href="http://nasavoda.rs" title="nasa voda" target="_blank"><img src="img/projects/nasaVoda.jpg" alt="nasa voda project"/>
                        <h2>Naša Voda</h2>
                        <p>
                            <i>Official website created for the company "Naša Voda".
                            Contains the basic information about the company and their
                                products.</i>
                        </p></a>
					</div>
					<div id="rightProjectImage">
						<a href="http://zolatrade.lazardjokovic.com" title="zolatrade" target="_blank"><img src="img/projects/zolaTrade.jpg" alt="nasa voda project"/>
                        <h2>ZolaTrade</h2>
                        <p>
                            <i>ZolaTrade is online store where you can buy video games.
                                You can also post comments, like and dislike products...</i>
                        </p></a>
					</div>
				</div>
			</div>
		</section>
		<footer>
			<div id="getInTouch">
                <a href="mailto:contact@lazardjokovic.com"><button id="getInTouchBtn">Get in touch</button></a>
			</div>
			<div id="myName">
                <strong>Lazar Djokovic - <a href="http://lazardjokovic.com" target="_blank" title="lazardjokovic.com">lazardjokovic.com</a></strong>
			</div>
			<div id="allRightsReserved">
			     <strong>2017 ALL RIGHTS RESERVED</strong>
			</div>
			<div id="socialNetworks">
                    <li><a href="https://www.facebook.com/profile.php?id=100000430518730"  target="_blank"><img src="img/socialNetworks/fb.png" title="facebook"/></a></li>
                    <li><a href="https://twitter.com/lazardjokovic95" target="_blank"><img src="img/socialNetworks/twitter.png" title="twitter" /></a></li>
                    <li><a href="https://github.com/LazarDjokovic" target="_blank"><img src="img/socialNetworks/github.png" title="github"/></a></li>
			</div>
		</footer>
	</body>
</html>